...
class shopProductbrandsPluginFrontendBrandAction extends shopFrontendCategoryAction
{
    public function execute()
    {
        ...
        if (!$brand) {
            $value_id = $values_model->getValueId($feature_id, $brand_url);
            if (!$value_id) {
        ...
        $sql = "SELECT cp.category_id, COUNT(DISTINCT cp.product_id) FROM shop_category_products cp
                ...
                WHERE pf.feature_id = ".(int)$feature_id." AND pf.feature_value_id = ".(int)$brand['id']."
                GROUP BY cp.category_id";
        $categories_count = $feature_model->query($sql)->fetchAll('category_id', true);
        ...
        $c_setting = wa()->getSetting('categories_filter', '', $key);
        if ($c_setting == -1) {
        ...
        $c->filters(array($feature['code'] => $brand['id']));
        ...
        if ($brand['filter']) {
            ...
            $brand_value_ids = $c->getFeatureValueIds();

            $filters = array();
            foreach ($filter_ids as $fid) {
                if ($fid == 'price') {
                    $range = $c->getPriceRange();
        ...
        if (wa()->getSetting('title_h1', '', $key) && $brand['title']) {
            $brand_name = $brand['title'];
        }
        $this->view->assign('title', htmlspecialchars(!empty($category) ? $category['name'] : $brand_name));
        ...
        if ($template = $plugin->getSettings('brand_theme_template')) {
